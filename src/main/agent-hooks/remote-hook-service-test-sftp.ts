...
export function createRemoteHookTestSftp(initialFiles: Record<string, string> = {}): {
  sftp: SFTPWrapper
  fs: FakeRemoteHookFilesystem
} {
  const fs: FakeRemoteHookFilesystem = {
  ...
  // oxlint-disable-next-line typescript/consistent-type-assertions -- SAFETY: The fake implements the SFTP methods exercised by these installer tests.
  const testSftp = sftp as unknown as SFTPWrapper
  return { sftp: testSftp, fs }
}
