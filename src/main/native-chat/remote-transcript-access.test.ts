...
function memoryProvider(
  ...
  const listFiles = vi.fn(async (): Promise<string[]> => [])
  const unwatch = vi.fn()
  ...
  return {
    // oxlint-disable-next-line typescript/consistent-type-assertions -- SAFETY: This in-memory fake implements every filesystem operation exercised by transcript access.
...
describe('remote TraeX transcript access', () => {
  it('reads an exact paginated tail through bounded positional reads', async () => {
    ...
    expect(
      harness.readFileRange.mock.calls.every(
        (call) => typeof call[2] === 'number' && call[2] <= 256 * 1024
      )
    ).toBe(true)
  })
